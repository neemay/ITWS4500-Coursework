...
app.get('/', (req, res) => res.sendFile('public/lab10.html', {root: __dirname}));

app.get('/visualize', (req, res) => res.sendFile('public/visualize.html', {root: __dirname}));
...
//Function to get the queries used to collect tweets
app.get('/queries', function(req, res) {
  console.log("Getting the queries used to save tweets.");
  //Query to get all query terms from the tweets in the database
  Tweet.find({}, function(err, docs) {
    if(err) throw err;
    var queries = [];
    for(var i in docs) {
      queries.push(docs[i].query);
    }
    res.send(queries);
  });
});
